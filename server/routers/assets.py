...
from lib.project_paths import resolve_asset_file, user_asset_relpath
from server.auth import CurrentUser, create_media_token
...
def _serialize(asset, *, user_id: str | None = None) -> dict:
    data = {
        "id": asset.id,
        "type": asset.type,
        "name": asset.name,
    ...
    if user_id and asset.image_path:
        rel = asset.image_path
        filename = Path(rel).name
        if rel.startswith(f"users/{user_id}/assets/"):
            data["image_url"] = f"/api/v1/global-assets/{asset.type}/{filename}"
            data["media_token"] = create_media_token(
                user_id,
                asset_path=rel,
            )
    return data
...
async def _save_upload(file: UploadFile, asset_type: str, user_id: str, _t: Translator) -> str:
    ...
    root = get_project_manager().get_user_assets_root(user_id) / asset_type
    uid = uuid.uuid4().hex
    target = root / f"{uid}{ext}"
    await asyncio.to_thread(target.write_bytes, data)
    return user_asset_relpath(user_id, asset_type, f"{uid}{ext}")


def _delete_global_asset_file(rel_path: str, *, user_id: str | None = None) -> None:
    pm = get_project_manager()
    path = resolve_asset_file(pm.projects_root, rel_path, user_id=user_id)
    if path is None:
        return
    try:
        path.unlink()
...
async def list_assets(
    ...
    async with async_session_factory() as s:
        ...
        return {"items": [_serialize(a, user_id=_user.id) for a in items]}
...
async def get_asset(asset_id: str, _user: CurrentUser, _t: Translator):
    async with async_session_factory() as s:
        ...
        return {"asset": _serialize(a, user_id=_user.id)}
    ...
    try:
        async with async_session_factory() as s:
            ...
            try:
                ...
                await s.rollback()
                if image_path:
                    _delete_global_asset_file(image_path, user_id=_user.id)
                    image_path = None
...
async def delete_asset(asset_id: str, _user: CurrentUser, _t: Translator):
    async with async_session_factory() as s:
        ...
        if not a:
            raise HTTPException(status_code=404, detail=_t("asset_not_found", name=asset_id))
        if a.image_path:
            _delete_global_asset_file(a.image_path, user_id=_user.id)
...
async def replace_image(
    ...
    try:
        ...
        _delete_global_asset_file(new_path, user_id=_user.id)
        raise

    # 4) DB 更新成功后才删除旧文件
    if old_path and old_path != new_path:
        _delete_global_asset_file(old_path, user_id=_user.id)
...
async def from_project(
    ...
    if source_sheet_path is not None:
        ...
        root = get_project_manager().get_user_assets_root(_user.id) / req.resource_type
        uid = uuid.uuid4().hex
        target = root / f"{uid}{ext}"
        await asyncio.to_thread(shutil.copyfile, source_sheet_path, target)
        new_image_path = user_asset_relpath(_user.id, req.resource_type, f"{uid}{ext}")
    ...
    try:
        async with async_session_factory() as s:
            ...
            if existing is not None:
                ...
                await s.refresh(a)
                if old_image:
                    _delete_global_asset_file(old_image, user_id=_user.id)
            else:
                try:
                    a = await repo.create(
                    ...
                    await s.rollback()
                    if new_image_path:
                        _delete_global_asset_file(new_image_path, user_id=_user.id)
                    raise HTTPException(
                        status_code=409,
...
async def apply_to_project(
    ...
    for asset_id in req.asset_ids:
        ...
        if a.image_path:
            src = resolve_asset_file(project_manager.projects_root, a.image_path, user_id=_user.id)
