"""Custom provider repository（按 user_id 隔离）。
...
class CustomProviderRepository(UserScopedRepository):
    """自定义供应商 + 模型 CRUD。"""

    async def _owned_provider(self, provider_id: int) -> CustomProvider | None:
        stmt = self._scope_query(select(CustomProvider).where(CustomProvider.id == provider_id), CustomProvider)
        result = await self.session.execute(stmt)
        return result.scalar_one_or_none()

    async def _owned_model(self, model_id: int) -> CustomProviderModel | None:
        stmt = select(CustomProviderModel).where(CustomProviderModel.id == model_id)
        result = await self.session.execute(stmt)
        model = result.scalar_one_or_none()
        if model is None:
            return None
        provider = await self._owned_provider(model.provider_id)
        return model if provider is not None else None
    ...
    async def get_provider(self, provider_id: int) -> CustomProvider | None:
        return await self._owned_provider(provider_id)

    async def list_providers(self) -> list[CustomProvider]:
        stmt = self._scope_query(select(CustomProvider).order_by(CustomProvider.id), CustomProvider)
        result = await self.session.execute(stmt)
        return list(result.scalars())

    async def update_provider(self, provider_id: int, **kwargs) -> CustomProvider | None:
        """更新供应商字段。返回更新后的对象，若不存在或不属于当前用户返回 None。"""
        provider = await self._owned_provider(provider_id)
        if provider is None:
            return None
    ...
    async def list_models(self, provider_id: int) -> list[CustomProviderModel]:
        if await self._owned_provider(provider_id) is None:
            return []
        stmt = (
    ...
    async def update_model(self, model_id: int, **kwargs) -> CustomProviderModel | None:
        """更新模型字段。返回更新后的对象，若不存在或不属于当前用户返回 None。"""
        model = await self._owned_model(model_id)
        if model is None:
            return None
        for key, value in kwargs.items():
    ...
    async def list_all_enabled_models(self) -> list[CustomProviderModel]:
        """跨当前用户所有供应商获取全部已启用模型。"""
        provider_ids = [p.id for p in await self.list_providers()]
        if not provider_ids:
            return []
        stmt = (
            select(CustomProviderModel)
    ...
    async def get_model_by_ids(self, provider_id: int, model_id: str) -> CustomProviderModel | None:
        """根据供应商 ID 和模型 ID 获取模型（校验父 provider 归属）。"""
        if await self._owned_provider(provider_id) is None:
            return None
