...
- 多选枚举字段不在 prompt 里写"如何选"判据，避免把人的镜头审美灌给 LLM；
  让模型按画面内容自行决定。
- 对直接驱动视频的动作描述设置防退化最低长度，并按镜头时长给出建议篇幅；不设限制细节的硬上限。
...
from lib.script_models import GENERATED_VIDEO_ACTION_MIN_LENGTH
...
# video_prompt.action 写作指导：i2v 契约（首帧已定、只写运动）+ 时间分段 + 微观物理细节。
_ACTION_WRITING_GUIDE = f"""首帧画面已定格主体、场景与风格，这段文字是直接发给视频模型的核心运动指令：只描述该时长内发生的运动与变化，不复述静态外观；镜头运动专由 camera_motion 字段承载，action 只写画面内部的运动。
   **篇幅底线**：action 的有效描述不得少于 {GENERATED_VIDEO_ACTION_MIN_LENGTH} 个字符；这是防止几十字退化输出的下限，不是目标值。按时长建议写足：1–4 秒约 120–180 字，5–6 秒约 180–260 字，7–10 秒约 260–380 字，超过 10 秒至少 380 字并增加过程阶段。宁可补充同一动作的微观演变，也不要只写动作梗概；不存在限制细节的硬上限。
   **时间过程**：围绕一条主要动作线，按「起初状态 → 动作启动 → 中段发展 → 末段收束 / 末帧状态」写清连续演变，用「起初、随后、中段、末段」等自然时序连接；不要写精确秒数时间码，也不要把多个互不相关的大动作硬塞进同一镜头。
   **细节层次**：至少覆盖主体的肢体 / 手势 / 视线 / 表情与呼吸变化、接触物的受力与反馈、前中后景环境联动（衣摆、发丝、尘埃、雨水、烟雾、反射）、材质与光影随运动产生的连续变化，以及末帧的停留姿态和必须保持稳定的轴线 / 比例 / 外形。所有动词都应物理可观察，避免「陷入、回忆、意识到、决定」等内心动词。
   **动作密度**：细节多不等于动作多。短镜头把一个动作写深；长镜头才增加动作阶段。运动优先平滑、连贯、可执行，明确加速 / 减速、幅度、方向、惯性、遮挡与回弹等变化，避免突然位移、形体漂移和无因抖动。
   6 秒正例：「起初，子弹在画面中央保持近乎静止的悬浮，只出现幅度极小的上下漂移；随后沿纵轴缓慢起转，转速由几乎不可察觉逐渐提升为稳定匀速，铜壳高光从右侧边缘连续滑向正面，再越过弧面落入左侧暗部。旋转中，弹壳底部的微弱火光呈低频呼吸式明灭，每次亮起都带动一圈轻薄热浪向外扩散，周围细小金属粉尘随气流缓慢偏移；红漆弹头上的划痕在高光扫过时依次显现又隐没。末段转速平滑降低，漂移幅度随之收窄，子弹重新稳定悬停在原位；整个过程运动连续克制，轴线、比例和外形始终保持一致。」——只有一条旋转主线，却写清启动、稳定、环境反馈、材质光影和末帧收束。
   反例（过短）：「子弹缓慢旋转，高光移动，底部火光闪烁。」——只有动作摘要，未说明动作怎样启动和结束、速度与幅度如何变化、环境如何响应，也没有末帧稳定要求。
   反例（不可拍）：「林清像蝴蝶般飞舞，思绪在过去与现在之间快速切换。」——「思绪切换」不是可拍摄的运动；「像蝴蝶般」是修辞，不是动作描述。"""
