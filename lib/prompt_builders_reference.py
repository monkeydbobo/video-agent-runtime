...
from lib.script_models import GENERATED_REFERENCE_SHOT_TEXT_MIN_LENGTH
...
a. **shots**：{duration_guide_line}
    - `text`：这段文字将直接驱动该镜头的视频生成（语言遵循上方"输出语言"约束）。每个 shot 不得少于 {GENERATED_REFERENCE_SHOT_TEXT_MIN_LENGTH} 个字符；这是防退化下限，不是目标值。按时长建议写足：1–3 秒约 180–240 字，4–6 秒约 240–360 字，7–10 秒约 360–520 字，超过 10 秒至少 520 字。按「景别 → 构图 → 运镜 → 画面内容」四要素依次组织，宁详勿略：
        - 景别：大全景 / 全景 / 中景 / 近景 / 特写，及拍摄角度（俯拍 / 仰拍 / 平视）。
        - 构图：主体在画面中的位置、前景与背景的关系（如中心构图、对角线构图、以公路 / 廊柱作引导线）。
        - 运镜：机位与镜头运动（固定机位 / 跟随 / 推近 / 拉远 / 摇移），含镜头内焦点主体的变更。
        - 画面内容：占篇幅大头——围绕一条主要动作线，按「起初状态 → 动作启动 → 中段发展 → 末段收束 / 末帧状态」写清连续过程。逐层写每个主体的肢体 / 手势 / 视线 / 神态与呼吸变化、物件受力和反馈、前中后景环境联动（人群、天气、衣摆、尘埃、烟雾、反射）、材质和光影随运动的变化；必要时补充速度、幅度、方向、惯性、遮挡、回弹与动态模糊，末尾交代停留姿态、稳定要求和氛围基调。短镜头把一个动作写深，更长镜头才增加动作阶段；不要为凑篇幅堆叠互不相关的大动作。
        - 角色 / 场景 / 道具仅用 `@[名称]` 引用——外貌、服装、场景陈设等静态外观由参考图承担，**不要**在文本里描写；动作、姿态、互动与环境动态则写得越具体越好。动词应描述物理可观察动作（伸手 / 转身 / 摩挲 / 投向 / 收紧），避免「陷入 / 回忆 / 意识到 / 决定」等内心动词。
        - 6 秒正例：「景别：中景，轻微仰拍。构图：@[角色A] 居画面中央偏右，@[场景A] 的窗棂在前景形成纵向遮挡，案几边缘斜向延伸至主体，背景空间保持轻微虚化。运镜：机位从固定状态开始极缓推近，焦点先锁在 @[角色A] 的手部，随后平滑转移到双眼，推进速度在末段逐渐放缓。画面内容：起初，@[角色A] 静立半拍，胸口随克制的呼吸轻微起伏，右手悬停在木窗前；随后指尖接触窗框并持续发力，木窗缓慢向外转开，衣袖和发梢被穿堂风依次掀起。中段他低头看向掌中的 @[道具A]，拇指沿其边缘轻轻摩挲，另外四指逐节收紧，目光再由 @[道具A] 平稳抬向窗外；风力带动前景帘角先扬起后回落，细尘穿过斜射光束，案上烛焰向同一方向压低并连续明灭，移动的窗影缓慢掠过他的面部。末段推镜和手部动作同时减速，他保持握住 @[道具A] 的姿势停在窗前，呼吸逐渐平稳，视线方向、人物比例与肢体结构保持稳定，整体氛围压抑而克制。」
