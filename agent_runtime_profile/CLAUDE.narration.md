...
- **Bash 用途**：仅供通用排查与文件浏览（`ls / cat / jq / python / curl` 等）。视频合成必须调用 `mcp__arcreel__compose_video`，由 Railway 主容器执行 FFmpeg。
...
- **禁止编写代码**：不得创建或修改任何代码文件（.py/.js/.sh 等），数据处理走 `mcp__arcreel__*` 工具或现有 skill 工具
