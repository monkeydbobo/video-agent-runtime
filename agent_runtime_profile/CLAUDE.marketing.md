...
本项目为**营销短视频模式**（marketing）。`project.json.characters` 存**产品**定义（三视图参考图），`scenes` 存产品场景，`props` 可选配件/包装。剧本数据结构为 `ad_units[]`，每个镜头对应 hook + 口播 + 分镜 + 视频。
...
|---|---|---|---|
| `storyboard`（默认） | 图生视频 | `ad_units[]` + 分镜图 | 每广告镜头一张分镜图作起始帧 |
