...
async def test_list_orders_by_id(async_session) -> None:
    repo = AgentCredentialRepository(async_session)
    a = await repo.create(user_id=_USER, preset_id="deepseek", display_name="A", base_url="u", api_key="k1")
    b = await repo.create(user_id=_USER, preset_id="kimi", display_name="B", base_url="u", api_key="k2")
    await async_session.flush()
    items = await repo.list_for_user(_USER)
...
async def test_set_active_makes_others_inactive(async_session) -> None:
    repo = AgentCredentialRepository(async_session)
    a = await repo.create(user_id=_USER, preset_id="x", display_name="A", base_url="u", api_key="k1")
    b = await repo.create(user_id=_USER, preset_id="y", display_name="B", base_url="u", api_key="k2")
    await async_session.flush()
    await repo.set_active(a.id, _USER)
    await async_session.flush()
    active = await repo.get_active(_USER)
    assert active is not None and active.id == a.id

    await repo.set_active(b.id, _USER)
    await async_session.flush()
    active = await repo.get_active(_USER)
...
async def test_delete_active_raises(async_session) -> None:
    repo = AgentCredentialRepository(async_session)
    a = await repo.create(user_id=_USER, preset_id="x", display_name="A", base_url="u", api_key="k")
    await async_session.flush()
    await repo.set_active(a.id, _USER)
    await async_session.flush()
    with pytest.raises(ValueError, match="active"):
        await repo.delete(a.id, _USER)
...
async def test_delete_inactive_works(async_session) -> None:
    ...
    a = await repo.create(user_id=_USER, preset_id="x", display_name="A", base_url="u", api_key="k")
    await async_session.flush()
    assert await repo.delete(a.id, _USER) is True
    await async_session.flush()
...
async def test_delete_nonexistent_returns_false(async_session) -> None:
    repo = AgentCredentialRepository(async_session)
    assert await repo.delete(9999, _USER) is False
...
async def test_set_active_unknown_id_raises(async_session) -> None:
    ...
    with pytest.raises(ValueError, match="not found"):
        await repo.set_active(9999, _USER)
...
async def test_update_partial(async_session) -> None:
    ...
    a = await repo.create(user_id=_USER, preset_id="x", display_name="A", base_url="u", api_key="k")
    await async_session.flush()
    updated = await repo.update(a.id, _USER, display_name="A2", model="m1")
...
async def test_get_active_when_none(async_session) -> None:
    repo = AgentCredentialRepository(async_session)
    assert await repo.get_active(_USER) is None
